...
def _clean_context(billing):
    ...
    return {
        ...
        "unpaid_bills": [_pick(b, UNPAID_BILL_FIELDS) for b in billing.get("unpaid_bills", [])],
        "due_date": billing.get("due_date"),
        "days_remaining": billing.get("days_remaining"),
        "pending_xendit": billing.get("pending_xendit"),
        "recent_payments": [_pick(p, PAYMENT_FIELDS) for p in billing.get("recent_payments", [])],
        "payment_methods": [
...
async def login(request: Request):
    ...
    if not DEBUG:
        if not body.get("name") and not body.get("last_receipt"):
            return JSONResponse(
                {"error": "Registered name or last receipt number is required"},
                status_code=400,
            )
    try:
        result = await api_client.customer_login(
...
def _relay_error(e):
    ...
    return JSONResponse(
        {
            "error": "Verification failed. Please contact support with this code.",
