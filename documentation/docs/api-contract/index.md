...
When an internal API key is provided (matching `INTERNAL_API_KEY` env var), permission checks are bypassed and the key resolves to `True` (internal). Some endpoints require `X-Staff-ID` header to specify the acting staff member when using internal auth.
