...
import pytest
from preflight import _add_column_sql, _modify_column_sql, decide
...
class TestColumns:
    def test_missing_column_fatal_with_ddl(self, engine, good_md):
        Table("users", good_md, Column("nickname", String(64)), extend_existing=True)
...
class TestTimeGuard:
    def test_time_named_non_datetime_fatal(self, engine, good_md):
        Table("users", good_md, Column("last_modified", String(64)), extend_existing=True)
        findings = run_decide(engine, good_md)
        assert any(f.kind == "fatal" and "last_modified" in f.message for f in findings)
...
class TestIndexes:
    ...
    def test_missing_manifest_index_created(self, engine, good_md):
        findings = decide(
            sa_inspect(engine),
            good_md,
            [("ix_users_name_phone", "users", ["name", "phone"], False)],
        )
        f = [f for f in findings if f.kind == "created_index"]
    ...
    def test_index_name_conflict_fatal(self, engine, good_md):
        ...
        assert any(f.kind == "fatal" and "ix_users_name_phone" in f.message for f in findings)

    def test_existing_unique_covered_by_column_set(self, engine, good_md):
        with engine.begin() as conn:
            conn.execute(sa_text("ALTER TABLE users ADD COLUMN email VARCHAR(64)"))
            conn.execute(sa_text("CREATE UNIQUE INDEX uq_users_email ON users (email)"))
        Table(
            "users",
            good_md,
    ...
    def test_redundant_prefix_dropped(self, engine, good_md):
        with engine.begin() as conn:
            ...
            conn.execute(sa_text("CREATE INDEX ix_users_name_phone ON users (name, phone)"))
        findings = run_decide(engine, good_md)
    ...
    def test_unique_index_never_dropped(self, engine, good_md):
        with engine.begin() as conn:
            conn.execute(sa_text("CREATE UNIQUE INDEX ux_users_name ON users (name)"))
            conn.execute(sa_text("CREATE INDEX ix_users_name_phone ON users (name, phone)"))
        findings = run_decide(engine, good_md)
    ...
    def test_unique_column_not_double_counted(self, engine, good_md):
        ...
        findings = run_decide(engine, good_md)
        created = [f for f in findings if f.kind == "created_index" and "email" in f.message]
        assert len(created) == 1

    def test_identical_indexes_not_dropped(self, engine, good_md):
        with engine.begin() as conn:
            conn.execute(sa_text("CREATE INDEX ix_users_name_a ON users (name)"))
            conn.execute(sa_text("CREATE INDEX ix_users_name_b ON users (name)"))
        findings = run_decide(engine, good_md)
        assert not any(f.kind == "dropped_index" for f in findings)
    ...
    def test_covered_by_differently_named_index(self, engine, good_md):
        with engine.begin() as conn:
            conn.execute(sa_text("CREATE INDEX ix_other ON users (name, phone)"))
        findings = decide(
            sa_inspect(engine),
            good_md,
            [("ix_users_name_phone", "users", ["name", "phone"], False)],
    ...
    def test_unique_plus_index_column_matches_sqlalchemy_ddl(self, engine, good_md):
        ...
        with engine.begin() as conn:
            conn.execute(sa_text("ALTER TABLE users ADD COLUMN email VARCHAR(64)"))
            conn.execute(sa_text("CREATE UNIQUE INDEX ix_users_email ON users (email)"))
        Table(
            "users",
            good_md,
        ...
        findings = run_decide(engine, good_md)
        assert not any(f.kind == "fatal" for f in findings)
        assert not any(f.kind == "created_index" and "email" in f.message for f in findings)
