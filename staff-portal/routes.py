...
async def edit_customer(
    customer_id: int, request: Request, _=Depends(require_perms("can_enroll_customer"))
):
    try:
        data = await request.json()
    except Exception:
...
async def undo_payment(
    payment_id: int, request: Request, _=Depends(require_perms("can_drop_payment"))
):
    try:
        data = await request.json()
    except Exception:
