...
async def xendit_webhook(request: Request):
    ...
    if not isinstance(data, dict):
        data = {}
    logger.info(
        f"Xendit webhook received: event={data.get('event')}, status={data.get('data', {}).get('status', 'unknown')}"
    )
    ...
    if not tx:
        result = await session().execute(
            select(XenditTransaction).where(
                XenditTransaction.xendit_pr_id == callback.get("id", "")
            )
        )
        tx = result.scalar_one_or_none()
    if not tx:
        logger.warning(
            f"Xendit webhook: transaction not found for ref={reference_id} id={callback.get('id')}"
        )
        return {"received": True}

    logger.info(
        f"Found transaction: id={tx.id} cust={tx.customer_number} amount={tx.amount} current_status={tx.status}"
    )

    tx.status = status

    if status in ("PAID", "SUCCEEDED", "COMPLETED"):
        result = await session().execute(select(Staff).where(Staff.username == "xendit"))
